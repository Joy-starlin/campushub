        ...
        <section className="py-12 bg-white">
          <div className="container mx-auto px-4">
            <motion.div
              ...
              className="text-center mb-14"
            >
              <span className="inline-block text-xs font-black uppercase tracking-[0.2em] text-bugema-accent mb-3">Full Platform</span>
              <h2 className="text-3xl lg:text-4xl font-black text-gray-900">
                Every feature, <span className="text-bugema-blue">at a glance</span>
              </h2>
              <p className="mt-4 text-gray-500 max-w-lg mx-auto text-base leading-relaxed">
                A complete look at everything Bugema Hub has available for students, faculty, and alumni.
              </p>
            </motion.div>

            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
              {[
                { icon: Rss,           label: "News Feed",            href: "/events/news",       color: "text-blue-500" },
                { icon: Calendar,      label: "All Events",           href: "/events",             color: "text-blue-600" },
                { icon: Video,         label: "Virtual Events",       href: "/events/virtual",     color: "text-indigo-500" },
                { icon: Users,         label: "Club Directory",       href: "/clubs",              color: "text-purple-600" },
                { icon: MessageCircle, label: "Club Live Chat",       href: "/clubs/chat",         color: "text-violet-500" },
                { icon: BookOpen,      label: "Study Groups",         href: "/study-groups",       color: "text-purple-400" },
                { icon: ShoppingBag,   label: "Marketplace",          href: "/marketplace",        color: "text-green-600" },
                { icon: SearchCheck,   label: "Lost & Found",         href: "/lost-found",         color: "text-emerald-500" },
                { icon: Car,           label: "Ride Sharing",         href: "/rides",              color: "text-teal-500" },
                { icon: MapPin,        label: "Directions & Map",     href: "/directions",         color: "text-green-400" },
                { icon: MessageSquare, label: "Anonymous Feedback",   href: "/feedback",           color: "text-sky-500" },
                { icon: FileText,      label: "PDF Resources",        href: "/resources",          color: "text-cyan-600" },
                { icon: Briefcase,     label: "Jobs & Internships",   href: "/jobs",               color: "text-orange-500" },
                { icon: GraduationCap, label: "Alumni Network",       href: "/alumni",             color: "text-amber-600" },
                { icon: School,        label: "Universities",         href: "/universities",       color: "text-orange-400" },
                { icon: HandHeart,     label: "Mentorship",           href: "/mentorship",         color: "text-rose-400" },
                { icon: BarChart3,     label: "Leaderboard",          href: "/leaderboard",        color: "text-yellow-600" },
                { icon: Medal,         label: "Badges & Achievements",href: "/badges",             color: "text-yellow-500" },
                { icon: Bell,          label: "Notifications",        href: "/notifications",      color: "text-amber-500" },
                { icon: CheckCircle,   label: "Verified Badges",      href: "/verified-badges",    color: "text-green-500" },
              ].map((feat, i) => {
                ...
                return (
                  ...
                  >
                    <Link
                      href={feat.href}
                      className="group flex items-center gap-3 p-4 rounded-xl border border-gray-100 bg-gray-50/50 hover:bg-white hover:border-gray-200 hover:shadow-md transition-all duration-200"
                    >
                      <div className="flex-shrink-0 w-9 h-9 rounded-lg bg-white border border-gray-100 shadow-sm flex items-center justify-center group-hover:scale-110 transition-transform duration-200">
                        <FIcon className={`w-4 h-4 ${feat.color}`} />
                      </div>
                      <span className="text-sm font-semibold text-gray-700 group-hover:text-bugema-blue leading-tight transition-colors">
                        {feat.label}
                      </span>
                    </Link>
                  </motion.div>
